...
module Pwb
  module ImagesHelper
    ...
    # Assumes sized versions exist at path/image-WIDTH.ext (convention from seed_images:optimize)
    # @param url [String] The image URL
    # @return [String] srcset attribute value
    def generate_external_srcset(url)
      # Only generate responsive srcset for trusted domains where we know structure exists
      return "#{url} 1280w" unless trusted_webp_source?(url)

      ext = File.extname(url)
      base = url.chomp(ext)
      
      # Dynamically build srcset from configured widths
      # We assume the seed generation task has created all these variants
      sources = Pwb::ResponsiveVariants::WIDTHS.map do |width|
        # For the largest size (1280), we typically use the original file
        # But if we strictly followed the naming convention everywhere, it would be -1280.
        # For now, let's assume the original URL serves as the largest variant (often ~1280 or larger)
        if width >= 1280
          "#{url} #{width}w"
        else
          "#{base}-#{width}#{ext} #{width}w"
        end
      end
