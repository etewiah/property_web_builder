...
module PwbTenant
  class RentalListing < ApplicationRecord
    ...
    belongs_to :realty_asset, class_name: 'PwbTenant::RealtyAsset'
    ...
    def activate!
      transaction do
        realty_asset.rental_listings.where.not(id: id).update_all(active: false)
    ...
    def only_one_active_per_realty_asset
      if realty_asset && realty_asset.rental_listings.active_listing.where.not(id: id).exists?
    ...
    def ensure_active_listing_visible
      ...
      update_column(:archived, false) if active? && archived?
    end
