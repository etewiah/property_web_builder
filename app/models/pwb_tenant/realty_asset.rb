...
module PwbTenant
  ...
  #   - PwbTenant::SaleListing (for properties being sold)
  #   - PwbTenant::RentalListing (for properties being rented)
  #
  # For read operations, use PwbTenant::ListedProperty (materialized view) instead,
  ...
  class RealtyAsset < ApplicationRecord
    ...
    # Associations
    has_many :sale_listings, class_name: 'PwbTenant::SaleListing', foreign_key: 'realty_asset_id', dependent: :destroy
    has_many :rental_listings, class_name: 'PwbTenant::RentalListing', foreign_key: 'realty_asset_id', dependent: :destroy
    ...
    def refresh_properties_view
      PwbTenant::ListedProperty.refresh
    ...
    def ensure_slug_uniqueness
      ...
      while self.class.where(slug: slug, website_id: website_id).where.not(id: id).exists?
    ...
    def build_slug_base
      ...
      parts << SecureRandom.hex(4) if parts.empty?
